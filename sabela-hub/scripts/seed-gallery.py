...
CURATION = [
    {
        ...
        # The notebook's saved outputs reference /api/asset (a live-editor
        # endpoint the static gallery can't serve), so the 3D viewers render
        # empty. Repoint them at the author's committed public assets so the
        # gallery page shows the real models. Rendered output only; source.md
        # (the fork artifact) stays verbatim.
        "assets": {
...
def main():
    ...
    for c in CURATION:
        ...
        write(
            os.path.join(sdir, "index.html"),
            page(c["title"], c["author"], c["slug"], render_body(rendered)),
        )
        write(
