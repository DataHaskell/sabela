...
spec = describe "G1 compile-gated notebook writes" $ do
    it "a non-compiling insert leaves the notebook, generation, and live session untouched" $ do
    ...
    it "a compiling insert commits and runs exactly as before (no gate-caused behaviour change)" $ do
        requireLiveIntegration
    ...
    it "a dep-adding cell that fails to compile never triggers the live install/restart" $ do
        requireLiveIntegration
    ...
    it "the infra-failure path (gate build budget breached) returns the infra verdict and commits nothing" $ do
        requireLiveIntegration
        withBuildTimeout "1" $
    ...
    it "replays the live_test4 hole probe: rejected with a diagnostic, notebook still has only cell 0" $ do
        requireLiveIntegration
