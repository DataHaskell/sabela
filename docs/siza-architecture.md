...
| Check vetting | `vetVerdictAgainst`, `Agent/Check/Vet.hs` | `Check/Gate.hs` | A check no perturbation can falsify proves nothing | "Gate" here names vet refusals; the compile gate is elsewhere. Build the scope with `scopeForCheck`: the reference gate must see every name, and only the typing budget is capped — capping the names made verifiability depend on cell position |
| Repeated-verify memo | `memoHit`/`memoRecord`, `Agent/VerifyMemo.hs` | `Stack/Call.hs` (`runToolCall`) | A memoised pass is returned only under a seal (all cell hashes + `ksGen` identical), and only a pass is memoised | The seal cannot see a re-execution of an unchanged cell whose output depends on external IO; the `unchanged` field states the basis |
