...
scatterViewJs =
    TE.decodeUtf8
        $(makeRelativeToProject "static/src/widgets/scatter-view.js" >>= embedFile)

scatterDrawJs :: Text
scatterDrawJs =
    TE.decodeUtf8
        $(makeRelativeToProject "static/src/widgets/scatter-draw.js" >>= embedFile)

scatterRendererJs :: Text
scatterRendererJs =
    TE.decodeUtf8
        $(makeRelativeToProject "static/src/widgets/scatter.js" >>= embedFile)

{- | The widget runtime, in dependency order: framing and zoom maths, painting,
then the renderer that wires them to the canvas and the pointer.
-}
scatterWidgetJs :: Text
scatterWidgetJs = T.intercalate "\n" [scatterViewJs, scatterDrawJs, scatterRendererJs]

scatterDefs :: Text
scatterDefs =
    T.unlines
        [ "data ScatterOpts = ScatterOpts { soWidth :: SabelaBase.Int, soHeight :: SabelaBase.Int, soColor :: SabelaBase.String, soAlpha :: SabelaBase.Double, soRadius :: SabelaBase.Double, soSelColor :: SabelaBase.String, soTitle :: SabelaBase.String, soXLabel :: SabelaBase.String, soYLabel :: SabelaBase.String, soXBounds :: SabelaBase.Maybe (SabelaBase.Double, SabelaBase.Double), soYBounds :: SabelaBase.Maybe (SabelaBase.Double, SabelaBase.Double), soColorBy :: [SabelaBase.Double], soEqualAspect :: SabelaBase.Bool }"
        , "defScatter :: ScatterOpts"
        , "defScatter = ScatterOpts { soWidth = 560, soHeight = 360, soColor = \"#4a9eff\", soAlpha = 0.55, soRadius = 2, soSelColor = \"#e3116c\", soTitle = \"\", soXLabel = \"\", soYLabel = \"\", soXBounds = SabelaBase.Nothing, soYBounds = SabelaBase.Nothing, soColorBy = [], soEqualAspect = SabelaBase.False }"
