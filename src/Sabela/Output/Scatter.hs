...
scatterDefs =
    T.unlines
        ...
        , "scatterSelectWith :: String -> ScatterOpts -> [(Double, Double)] -> Input [Int]"
        , "scatterSelectWith name opts pts = Input { iValue = widgetRead name ([] :: [Int]), iShow = scatterRender name opts pts }"
