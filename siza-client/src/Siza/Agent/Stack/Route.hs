...
{- |
Blocked-write rerouting for the per-call pipeline [Episode]: which red cell
blocks a write, and how the reroute is disclosed to the model.
Formerly Siza.Agent.Loop.Route; its only consumer is Siza.Agent.Stack.Call.
-}
module Siza.Agent.Stack.Route (
    blockingCell,
    discloseRoute,
    routedRetryNote,
) where
