{- |
Grammar-card surfacing after installs and writes [Discovery].
NOT the search tool (that is Siza.Agent.DiscoverTool) and NOT the server
locator (Siza.Discover). Formerly Siza.Agent.Discover.
-}
module Siza.Agent.GrammarCards (
    GrammarMode (..),
    Dispatch,
    declaredPackages,
    declaresDepsCall,
