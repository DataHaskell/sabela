...
{- |
Startup check that the local Ollama binary and model are reachable
[Infrastructure]. Not a code gate: the parse/security gate is Siza.Preflight.
Formerly Siza.Agent.Preflight.
-}
module Siza.Agent.ProviderCheck (
    Preflight (..),
    classifyPreflight,
    preflightMessage,
    ensureOllama,
